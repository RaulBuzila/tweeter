...
//        }
    ...
    public static List readTweetFeed(int userid) throws ClassNotFoundException, SQLException {
        ...
        // 5. execute a query
        ResultSet rs = st.executeQuery("SELECT * FROM  tweet,\"user\" WHERE \"user\".k=tweet.fku and fku IN(SELECT \"FKR\" FROM relations WHERE \"FKU\"= "+ userid + ") ORDER BY tweet_date DESC ");
        // 6. iterate the result set and print the values
    ...
    public static int login(String user , String password) throws ClassNotFoundException, SQLException {
        ...
        // 5. execute a que0
        ResultSet rs = st.executeQuery("SELECT * FROM \"user\" where username='"+user+ "' AND password='"+password+ "'");
